...
def cutoff_countmin(y, n_buckets, b_cutoff, n_hashes):
    ...
    loss_avg = (loss_cf * np.sum(np.abs(y[:b_cutoff])) + loss_cm * np.sum(np.abs(y[b_cutoff:]))) / np.sum(np.abs(y))
...
def run_ccm_wscore(y, scores, score_cutoff, space_alloc, n_hashes, name):
    start_t = time.time()
    len_y_ccm = len(y[scores >  score_cutoff])
    n_cm_buckets = int((space_alloc*1e6-len_y_ccm*4)/(n_hashes*4))
...
def cutoff_countmin_wscore(y, scores, score_cutoff, n_cm_buckets, n_hashes):
    ...
    space = len(y_ccm) * 4 + n_cm_buckets * n_hashes * 4
    return loss_avg, space
